...
def _write_eval_md_fallback(metrics_json: Path, out_md: Path) -> Path:
    try:
    ...
    out_md.write_text(
        ...
        f"_Generated by fallback writer (report module unavailable or failed)._",
        encoding="utf-8",
    )
    return out_md
# ---------------------------------------------------------------------------
...
def evaluate(cfg: Dict, checkpoint: str = "models/best.pt",
    ...
    # markdown report (always produce a file)
    md_written = False
    try:
        from .report import generate_eval_report
        generate_eval_report(metrics_path, plots_dir / "evaluation.md")
        md_written = True
    except Exception:
        md_written = False

    if not md_written:
        _write_eval_md_fallback(metrics_path, plots_dir / "evaluation.md")
